...
class CarlaAckermannControl(CompatibleNode):
    ...
    def __init__(self):
        ...
        # Newer simple-pid versions expose a per-instance time function instead of
        # the older module-level hook used by the original CARLA package.
        pid_bootstrap_time = lambda: self.get_time() - 0.1
        pid_time = lambda: self.get_time()
        ...
        # use the correct simulation time for further calculations
        self.speed_controller.time_fn = pid_time
        self.accel_controller.time_fn = pid_time
